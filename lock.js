function _bpTerms(h){var D=document;try{if(window.top!==window.self&&window.top.document)D=window.top.document;}catch(e){}var old=D.getElementById('bpsy-terms-ov');if(old&&old.parentNode)old.parentNode.removeChild(old);var ov=D.createElement('div');ov.id='bpsy-terms-ov';ov.style.cssText='position:fixed;left:0;top:0;right:0;bottom:0;width:100%;height:100%;z-index:2147483000;background:#14110F;display:flex;flex-direction:column';var bar=D.createElement('div');bar.style.cssText='flex:0 0 auto;padding:10px 14px;border-bottom:1px solid rgba(201,169,106,.35);background:#1A1512';var bt=D.createElement('button');bt.textContent='\u2039 返回勾選';bt.style.cssText='background:#C9A96A;color:#1A1512;border:0;border-radius:8px;padding:9px 16px;font-size:14px;cursor:pointer;font-weight:600';bt.onclick=function(){if(ov.parentNode)ov.parentNode.removeChild(ov);};bar.appendChild(bt);var f=D.createElement('iframe');f.src='terms.html'+(h||'');f.style.cssText='flex:1 1 auto;width:100%;height:100%;border:0;background:#14110F';ov.appendChild(bar);ov.appendChild(f);D.body.appendChild(ov);}
